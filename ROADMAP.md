...
| KI-022 | ~~Low~~ Fixed | ~~`[blocks-prod: yes]` Backend API calls on local dev are inconsistently slow.~~ Fixed by R-7-02's perf detective cycle (PR #73, merged 2026-04-28). Root cause: Clerk Go SDK v2's `clerk.SetKey()` mutated `http.DefaultTransport`, contaminating the AWS SDK's first DDB call (1.8 s vs 9 ms after isolation). Fix: dedicated `http.Client` per SDK via `http.DefaultTransport.Clone()` in `backend/cmd/api/main.go::loadAWSConfig`, plus `localhost→127.0.0.1` (skip happy-eyeballs IPv6 fallback), auth `GetUser` cache, and `localstack/localstack` image pinned to `4.14.0`. Patterns generalised in CLAUDE.md lessons 22–26. | R-7-02 |
...
| KI-025 | ~~Medium~~ Fixed | ~~Daily-puzzle timer restarts from zero when the player navigates back to the menu and re-opens the same daily puzzle.~~ Fixed by PR #112: the daily flow's displayed timer is now anchored on the server-issued `assignedAt` (set once on first GET per DP-19) instead of the client `useTimer` state. Display reads `Date.now() − assignedAt` directly and matches the leaderboard's `serverElapsedMs`; nothing to lose on navigate-back. | — |
