...
| KI-025 | Medium | Daily-puzzle timer restarts from zero when the player navigates back to the menu and re-opens the same daily puzzle. Elapsed time should persist (or resume from the prior elapsed value) for the duration of the daily slot, since the leaderboard ranks on total time. Likely root cause: `DailyGameBoard` initializes the timer on mount rather than rehydrating from a per-slot persisted start/elapsed value. | — |
