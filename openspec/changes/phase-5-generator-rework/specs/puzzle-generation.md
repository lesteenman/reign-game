...
- Per-rule necessity fixture: removing only that rule from the registry causes the fixture to fail.
- Cross-check on a 50-puzzle hand-crafted corpus: deductive solution equals brute solution. Hard failure on divergence (locked decision #8).
- Generated-corpus necessity: on a 500-puzzle corpus, every rule fires at least once. A rule that never fires is a bug or is redundant.
