...
3. **N range.** Commit to N=5..12; measure N=13..15 as stretch in Step 11.
...
- **AC-10. N_min interim = 5.** The package-level constant `N_min` is set to 5 as the interim floor. The final value is determined by the R-063 feasibility probe and may be raised (never lowered below 5) based on its data.
