...
**Severity:** info.
**The grill.** Six repo secrets land in CI: `CLERK_PUBLISHABLE_KEY` and `CLERK_SECRET_KEY` (existing, shared with Path A's CD pipeline) plus four new e2e-only secrets — `E2E_CLERK_TEST_ADMIN_EMAIL`, `E2E_CLERK_TEST_ADMIN_PASSWORD`, `E2E_CLERK_TEST_USER_EMAIL`, `E2E_CLERK_TEST_USER_PASSWORD`. GitHub by default does NOT expose secrets to PRs from forks, but Dependabot PRs run with a separate `dependabot` secret scope. If the e2e job is configured to run on Dependabot PRs and we forget to mirror the secrets to that scope, Dependabot e2e runs will fail mysteriously. Conversely, if we DO mirror, the credentials are slightly more exposed than necessary.
