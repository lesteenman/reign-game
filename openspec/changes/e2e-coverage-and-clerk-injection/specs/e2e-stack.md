...
- **ES-01.5** `task e2e:up:backend` MUST set `SQS_QUEUE_URL=http://localhost:4566/000000000000/puzzle-generation-e2e` (the e2e queue) — NOT the dev `puzzle-generation` queue. This is a lockstep requirement with ES-01: both the e2e backend (which enqueues replenish messages from `/api/admin/replenish`) and the e2e generator (which dequeues them) MUST point at the same queue. Pointing the backend at the dev queue causes replenish messages to disappear silently — no error surfaces because the dev queue exists and accepts the message, but the e2e generator polls a different queue and never consumes it. Caught and corrected in commit 62f0e50 after EC-05 went red on a stale Taskfile env.
