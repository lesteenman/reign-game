...
- **EC-01** `auth.spec.ts::admin route requires admin role` — Given an authenticated **non-admin** user, when they navigate to `/admin`, then they are redirected away or shown an unauthorized banner. Given an authenticated **admin** user (signed in with `E2E_CLERK_TEST_ADMIN_EMAIL`), when they navigate to `/admin`, then the admin pool widget renders.
