...
**Per-test sign-in** (used by auth + admin + dynamic-modes specs). Uses Clerk's email-ticket strategy via `@clerk/testing/playwright`'s `clerk.signIn({ page, emailAddress })` overload — Clerk's backend SDK mints a sign-in ticket for the email and the helper waits for `window.Clerk.user` to populate before returning. The original password-strategy snippet was replaced because that branch races (no built-in wait for the user object), and the email-ticket branch is the documented sign-in path for `@clerk/testing` post-v1:
```ts
...
test.beforeEach(async ({ page }) => {
  ...
  await clerk.signIn({
    page,
    emailAddress: process.env.E2E_CLERK_TEST_ADMIN_EMAIL!,
  });
});
```
...
**Env vars (6 secrets, mirrored to Actions + Dependabot per D3)**:
- `CLERK_PUBLISHABLE_KEY` (dev tenant, `pk_test_...`)
- `CLERK_SECRET_KEY` (dev tenant, `sk_test_...`)
- `E2E_CLERK_TEST_USER_EMAIL`
- `E2E_CLERK_TEST_USER_PASSWORD` (stored, not consumed by current email-ticket helpers — kept for future flexibility)
- `E2E_CLERK_TEST_ADMIN_EMAIL`
- `E2E_CLERK_TEST_ADMIN_PASSWORD` (stored, not consumed by current email-ticket helpers — kept for future flexibility)

Per chunk 4 lock-in: admin and user passwords are **distinct**. They are kept in the secret set even though the email-ticket helpers do not consume them, so a future password-strategy spec (e.g. exercising Clerk's hosted sign-in UI) does not need to re-provision credentials. Reusing the user password for the admin would shrink the secret count by one but leak the admin credential to any spec that logs the user password during a sign-in failure — the marginal cost of one extra secret is worth the blast-radius isolation.
