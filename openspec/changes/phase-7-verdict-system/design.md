...
5. **Verdict surface UI uses one component with `variant: 'completion' | 'skip'` — completion prominent, skip de-emphasized, with diverged button sets — RESOLVED (second grill).** Same `<VerdictSurface>` component, same data path, but different button rows per variant:
   - `completion`: two buttons — "Good puzzle" (up) and "Bad puzzle" (down). Prominent layout. Click submits the verdict; the parent overlay's existing Play Again / Home navigation stays functional independently. Silence on the verdict surface = "no opinion."
   - `skip`: three buttons — "Cancel" (Ghost variant; calls `onDismiss`, no API), "I hate this" (status PUT + verdict POST in parallel — the cull-from-corpus signal), "Just skip" (status PUT only, no verdict written). De-emphasized chrome (smaller padding, quieter prompt copy).

   No upvote-on-skip path: the high-signal click on skip is "this puzzle is bad, bailing," and an admin who liked the puzzle but bailed mid-attempt is rare and weakly-signaled — Cancel covers the "actually I want to keep playing" case. The `outcome` field distinguishes solved-rating from skipped-rating in the data. Reversal cost: low — both variants share state machine + service plumbing; changing the button set is component-local. Playtesting will validate the wording and visual weight.
...
1. **Verdict surface on skip vs completion** — RESOLVED in two grills:
   - First grill: single component with completion / skip variant, visually de-emphasized on skip.
   - Second grill: button sets diverge per variant — completion shows two buttons (Good/Bad), skip shows three buttons (Cancel / I hate this / Just skip). No upvote-on-skip path. Captured in §7 item 5 above and in spec FB-02.
