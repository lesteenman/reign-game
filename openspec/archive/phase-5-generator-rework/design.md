...
**Implementation note — R6 / R8 subsumption (found in R-064):** R6's precondition ("the row's only candidates are exactly K cells AND they share a region") is a strict superset of R3's row-axis firing precondition ("rowNeed equals cands count"). In the restart-from-Tier-1 fixed-point loop, R3 (Tier 1) always fires before R6 (Tier 3); after R3 places the marks, R2 eliminates the region's stale candidates. End-state matches R6's intended progress. Same for R8 at k=2: R8's preconditions imply R3-row firings for both rows involved, so R3 beats R8 to the same outcome. Both rules are retained in the registry for spec clarity and to keep their preconditions documentable, but they never contribute deductively distinct progress on reachable states. Their necessity tests are trace-level (removing the rule produces a trace without its events) rather than outcome-level.

**Implementation note — R9 disabled (found in R-066):** R9's precondition as written in input-spec §4.2 is **unsound at k=2**. Specifically, the inference "two regions' combined candidates in some line span only K cells → no other region's marks in that line outside those cells" does not hold when the K combined cells include mutually-adjacent columns: the adjacency constraint itself rules out both being marks, so the argument "those K cells hold all K marks for the two regions" fails. R-066 disables R9 in the registry; the original implementation is retained as `ruleRegionPairExclusionOriginal` in `rules.go` with a live regression test (`TestRuleR9_RegionPairExclusion_Original_Unsound`) pinning the failure case. Tightening the precondition (likely: require no column-adjacency among the K combined cells) is future-slice work.

**Consequence for difficulty classification:** `MaxTier` will never reach 3 via R6 alone (subsumed by R3), nor 4 via R8 alone (subsumed by R3) or R9 (disabled). **Tier 4 is currently unreachable; classification effectively caps at Tier 3 via R7 (genuine).** R-068's 500-puzzle corpus will show the actual per-rule firing rates and guide next-slice action: tighten R6/R8/R9 preconditions, move them to Tier 1 as R3-supersets, or delete them with a spec update.
