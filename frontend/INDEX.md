...
| UI primitives | Tamagui 2 RC config + provider + Vite compiler plugin all wired (#176 kickoff). Token/theme system mirrors `index.css`'s CSS custom properties as literal hex for compiler extraction. Per-component migration is incremental — chrome still uses inline `style={}` until each component lands its own Tamagui slice (Button migration is the first follow-up). One residual `className=` in `Cell.tsx` (keyframe-animation hook, not Tailwind). | Tamagui 2 RC primitives + theme tokens (per-component) |
