...
| UI primitives | Tamagui 2 RC config + provider + Vite compiler plugin all wired (#176 kickoff). Token/theme system mirrors `index.css`'s CSS custom properties as literal hex for compiler extraction. **Tamagui-migrated chrome: `Button.tsx` (#208), `IconButton.tsx` + `PageShell.tsx` (#176 PageShell slice), `Card.tsx` + `Spinner.tsx` + AdminLandingPage + DailyFlow cards (#176 chrome-cards slice).** Remaining inline `style={}` chrome (OfflineBanner, InstallButton, PostCompletionScreen modal panel, completion-overlay) migrates incrementally. One residual `className=` in `Cell.tsx` (keyframe-animation hook, not Tailwind). | Tamagui 2 RC primitives + theme tokens (per-component) |
...
- `PageShell.tsx` — *(2026-05-21: Tamagui migration in #176 PageShell slice)* top-of-page chrome (header, back button, subtitle, dark-mode toggle, auth slot, install button, offline banner). Outer container, header row, title (`<h1>` via `render` override), subtitle, and right-cluster are all Tamagui `styled(View|Text, ...)` primitives. Back button + dark-mode toggle use the new `IconButton`.
...
- `IconButton.tsx` — *(2026-05-21: #176 PageShell slice)* Tamagui-styled icon-only chrome button. 44×44 min tap target, transparent background, no shadow, hover-color shift. Variants: `ink` (full-opacity, primary affordances like back) / `muted` (muted color, secondary affordances like dark-mode toggle). Distinct from the action `<Button>` family — chrome icons would be visually overpowering with the shadow + translate press effect.
- `Card.tsx` — *(2026-05-21: #176 chrome-cards slice)* Tamagui-styled BRAND_GUIDELINES §5.5 card. Surface bg, 2px ink border, 10px radius, 3px ink shadow, centered flex column with 16px gap, maxWidth 480. Variants: `size: 'compact'` (24px padding, used by DailyFlow error/submit-error cards) / `size: 'large'` (32×24px padding, used by AdminLandingPage's anonymous + forbidden landing cards).
- `Spinner.tsx` — *(2026-05-21: #176 chrome-cards slice)* 32×32 brand-colored CSS-keyframe spinner with `role="status"` + `aria-label="Loading"` + `prefers-reduced-motion` opt-out. Extracted from DailyFlow's previously-inline `function Spinner()` to make it reusable across future async surfaces.
