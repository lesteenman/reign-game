...
| Tailwind | Gone (`tailwindcss` + `@tailwindcss/vite` removed in #176; `@import "tailwindcss"` removed from `index.css`; build precache -9 KB). The remaining `className=` in `Cell.tsx` is for a plain-CSS keyframe-animation hook, not Tailwind. | Already retired |
