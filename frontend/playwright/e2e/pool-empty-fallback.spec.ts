...
// Wait window between the first observed 404 and a confirming second 404.
// Auto-replenishment is async (generator fan-out via SQS) and can deliver a
// fresh row after drain hits its first 404 but before the test navigates.
// Requiring two consecutive 404s separated by this window pushes the race
// below navigation latency. Raise if CI starts flaking here again.
const DRAIN_SETTLE_MS = 800;

/**
 * Drain all ready 9#double rows by calling /api/puzzles/next until it 404s
 * twice in a row, separated by DRAIN_SETTLE_MS. Each successful (200)
 * response marks one more row served=true; a single 404 may race with an
 * in-flight async replenishment, so we wait briefly and re-check. If a
 * replenished row arrives in the settle window, the next iteration drains
 * it and resets the consecutive-404 counter.
 */
async function drainCombo(request: APIRequestContext): Promise<number> {
  ...
  throw new Error(
    `drain did not reach 2 consecutive 404s within ${DRAIN_MAX_ATTEMPTS} attempts — ` +
...
test.describe.serial("Pool-empty fallback", () => {
  test("drained combo surfaces user-readable empty-pool UX on /play", async ({
    ...
    // Arrange: drain 9#double until the backend returns 404 twice in a row
    // (with a settle window between checks to flush in-flight async
    // replenishments). The drain count is logged in the assertion message
    // below for diagnostic value when the spec fails.
