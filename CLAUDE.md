...
7. **Verify production request flow before deploy PRs:** Trace every URL the frontend calls and confirm it reaches the correct backend in production. Check CloudFront behaviors, API Gateway routes, and proxy config. Dev proxies mask routing issues that break in production. When adding new HTTP methods (PUT, POST) or new path prefixes (/admin), verify the API Gateway Terraform has matching method + integration resources — the dev proxy masks missing routes.
