...
15. **Run review-local before `gh pr create`, not after.** For every change — even surgical renames or find-and-replace scope — complete the 4-agent review loop and fix actionable findings *before* opening the PR. Opening a PR first and reviewing after means every finding becomes a noisy fixup commit and reviewers see un-reviewed code. Phase 4.5 caught a MAJOR Taskfile bug this way; without the pre-merge review it would have shipped. "It's too small to review" is not a valid reason to skip.
