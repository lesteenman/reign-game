...
// TestMutateBudgetExhaustionReturnsFailed: when a fixed-seed run
// would need more than maxMutations swaps to reach Solved, the mutator
// returns mutationFailed (not mutationSolved).
func TestMutateBudgetExhaustionReturnsFailed(t *testing.T) {
	t.Parallel()

	// Arrange — tight budget (1 mutation) should cause some attempts
	// to fail even though a larger budget would solve them.
	const n, k = 12, 1
	gLow, _ := New(n, k, WithSeed(123), WithMaxMutations(1))
	gHigh, _ := New(n, k, WithSeed(123), WithMaxMutations(50))

	// Act
	lowSolved := 0
	highSolved := 0
	for attempt := 0; attempt < 20; attempt++ {
		// Use parallel attempt counters; each generator has its own RNG,
		// so seeds differ per call — but identical between gLow/gHigh
		// only on first sample. This test checks the general shape.
		solLow, okLow := gLow.sampleSolution()
		solHigh, okHigh := gHigh.sampleSolution()
		if !okLow || !okHigh {
			continue
		}
		seedsLow := pairSeeds(solLow, n, k)
		seedsHigh := pairSeeds(solHigh, n, k)
		var rmLow, rmHigh [nMax][nMax]int8
		if !gLow.growRegions(seedsLow, &rmLow) {
			continue
		}
		if !gHigh.growRegions(seedsHigh, &rmHigh) {
			continue
		}
		gLow.regionOf = rmLow
		gHigh.regionOf = rmHigh
		if gLow.solveAndMutate(seedsLow) == mutationSolved {
			lowSolved++
		}
		if gHigh.solveAndMutate(seedsHigh) == mutationSolved {
			highSolved++
		}
	}

	// Assert — higher budget should be at least as effective. This is
	// a soft check (stochastic across 20 attempts with two different
	// generators), not a hard guarantee.
	if highSolved < lowSolved {
		t.Logf("observed: low-budget solved %d, high-budget %d — unusual but not deterministic-fatal",
			lowSolved, highSolved)
	}
	t.Logf("low-budget (K=1)  solved=%d/20", lowSolved)
	t.Logf("high-budget (K=50) solved=%d/20", highSolved)
}
