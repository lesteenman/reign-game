	...
	for {
		...
		if opts.Ctx != nil {
			if err := opts.Ctx.Err(); err != nil {
				return nil, fmt.Errorf("generating puzzle (constraint-aware): %w", err)
			}
		}
...
// constraintCandidate represents a frontier cell that can be assigned to a region
// during constraint-aware growth.
type constraintCandidate struct {
	r, c, rid  int
	regularity int // same-region neighbor count; lower = more irregular = preferred
}

// seedConstraintRegions initializes a region map from marker positions for
// constraint-aware growth. Returns the region map, region sizes, and total
// assigned cell count, or an error if seeding fails.
func seedConstraintRegions(solution [][]bool, gridSize, markersPerUnit int) (regionMap [][]int, regionSize []int, totalAssigned int, err error) {
	numRegions := gridSize
	...
	if markersPerUnit >= 2 {
		...
		pairs, pairErr := pairMarkersForDoubleQueens(solution, gridSize)
		if pairErr != nil {
			return nil, nil, 0, pairErr
		}
		...
		if markerIdx != numRegions {
			return nil, nil, 0, fmt.Errorf("found %d markers, expected %d", markerIdx, numRegions)
		}
	}
	...
	return regionMap, regionSize, totalAssigned, nil
}

// selectBestCandidate picks the best candidate from a list using a two-stage
// filter: first by largest region deficit (within 1), then by lowest regularity
// (most irregular boundary). Returns a random choice among the finalists.
func selectBestCandidate(candidates []constraintCandidate, regionSize, targets []int) constraintCandidate {
	// Find the largest deficit among candidates.
	bestDeficit := 0
	...
	var bestCandidates []constraintCandidate
	for _, cand := range candidates {
		deficit := targets[cand.rid] - regionSize[cand.rid]
	...
	var finalCandidates []constraintCandidate
	for _, cand := range bestCandidates {
		if cand.regularity <= minRegularity {
	...
	return finalCandidates[rand.IntN(len(finalCandidates))]
}
...
func (p *ConstraintAwarePipeline) growConstraintAware(solution [][]bool, gridSize, markersPerUnit int, deadline time.Time) [][]int {
	...
	regionMap, regionSize, totalAssigned, err := seedConstraintRegions(solution, gridSize, markersPerUnit)
	if err != nil {
		return nil
	}
	...
	for totalAssigned < total {
		...
		for r := 0; r < gridSize; r++ {
			for c := 0; c < gridSize; c++ {
				...
				for rid := range adjacentRegions {
					...
					candidates = append(candidates, constraintCandidate{r: r, c: c, rid: rid, regularity: sameNeighbors})
				}
			}
		}
				...
				for c := 0; c < gridSize; c++ {
					...
					for _, d := range dirs {
						...
						if nr >= 0 && nr < gridSize && nc >= 0 && nc < gridSize {
							rid := regionMap[nr][nc]
							if rid >= 0 {
								candidates = append(candidates, constraintCandidate{r: r, c: c, rid: rid, regularity: 0})
								break
							}
						}
					}
		...
		chosen := selectBestCandidate(candidates, regionSize, targets)
