...
Pure-TS puzzle domain. Four production files plus three unit tests.

## Responsibility

The "what is a Reign puzzle and when is one solved" domain knowledge — separated from React, IndexedDB, and the network. The architecture rule is strict: **the engine may only depend on external libs and import nothing from React, no `fetch`, no DOM**. It is the leaf of the dependency graph in `@reign/core`, shared by the web app and the future React Native client.
...
- **No React, no I/O, no DOM.** Enforced at design-time and review-time by the architecture skill.
