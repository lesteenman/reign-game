...
- Single Terraform state (`infra/`) currently targets one environment: **acc** (acceptance).
- The acc stack serves the live acceptance domain `reign.acc.steenman.me` (Phase 8, 2026-05-08).
- A **prod** env (`reign.steenman.me`) is planned but not yet wired — `infra/environments/prod/` contains only `.gitkeep`.
- Switching environments is currently a `var.environment` rename + a separate Terraform workspace / state-key swap; there is no per-env tfvars file. Future work: split into `infra/envs/acc/` and `infra/envs/prod/`.
...
├── outputs.tf                ← root outputs (cloudfront_url, api_gateway_url, frontend_bucket_name, frontend_distribution_id, puzzle_table_name, sqs_queue_url, generator_function_name, clerk_publishable_key_param_name)
...
├── environments/
│   └── prod/                 ← placeholder (.gitkeep only — prod not yet wired)
└── modules/
...
```
.github/workflows/
├── ci.yml                    ← PR: backend test/lint, frontend test, integration + e2e Playwright, terraform plan, security (gitleaks/govulncheck/npm audit). All actions use major-version tags (NOT SHA-pinned — see #113).
├── cd.yml                    ← merge to main: build Lambdas → terraform apply → fetch publishable key from SSM → build frontend with VITE_CLERK_PUBLISHABLE_KEY → sync to S3 → invalidate CloudFront
...
The root wires five modules together, passing outputs through as inputs:
