...
- **CLOUDWATCH log group is auto-created.** No `aws_cloudwatch_log_group` is defined here, so AWS auto-creates one on first invocation with `Never expire` retention. See `FINDINGS.md` (P1 cost smell).
...
- **DynamoDB IAM policy is wider than strictly needed by code today** — see `FINDINGS.md` (P2: re-audit on slice close).
